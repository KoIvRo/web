...
// Перехватчик для автоматической подстановки токенов
...
// Перехватчик для обработки ошибок и refresh токена
...
export const logout = () => {
  ...
  // Можно также вызвать API для logout на бэкенде
...
// Специфичные методы для аутентификации
// ПРЕДПОЛОЖИМ, ЧТО ВАШЕ API ИМЕЕТ ТАКИЕ ЭНДПОИНТЫ:
export const authAPI = {
  // Логин - предположим, что возвращает {access, refresh}
  login: (credentials) => apiService.post('/api/token/', credentials),
  
  // Регистрация - предположим формат
  ...
  // Проверка токена
  verify: (token) => apiService.post('/api/token/verify/', {token}),
};
